...
static inline unsigned int get_cpu_idle_times(unsigned int cpu)
{
	...
	const char *buf, size_t count)
{
	...
	j_dbs_info->prev_cpu_idle_up = get_cpu_idle_times(j);
...
static void dbs_check_cpu(int cpu)
{
    ...
    /* Check for frequency increase */
    total_idle_ticks = get_cpu_idle_times(cpu);
		   ...
		   unsigned int event)
{
	...
	for_each_cpu(j, policy->cpus) {
	    ...
	    j_dbs_info->prev_cpu_idle_up = get_cpu_idle_times(cpu);
...
static void lagfree_power_suspend(struct power_suspend *handler) {
    suspended = 1;
}
