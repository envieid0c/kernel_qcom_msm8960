...
static inline cputime64_t id_get_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
{
		      ...
		      const char *buf, size_t count)
{
	...
	struct cpu_dbs_info_s *dbs_info;
	dbs_info = &per_cpu(id_cpu_dbs_info, j);
	dbs_info->prev_cpu_idle = id_get_cpu_idle_time(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	cur_idle_time = id_get_cpu_idle_time(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	this_dbs_info->prev_cpu_idle = id_get_cpu_idle_time(cpu,
		   ...
		   unsigned int event)
{
	...
	for_each_cpu(j, policy->cpus) {
	    ...
	    j_dbs_info->prev_cpu_idle = id_get_cpu_idle_time(j,
...
static int cpufreq_gov_dbs_up_task(void *data)
{
	...
	if (policy->cur < input_event_min_freq) {
	    ...
	    this_dbs_info->prev_cpu_idle = id_get_cpu_idle_time(cpu, &this_dbs_info->prev_cpu_wall);
	}
